        ...
        if self.config.log.enable_summary:
            ...
            with tf.name_scope("train_summary"):
                ...
                with tf.name_scope("image_summary"):
                    ...
                    tf.summary.image("input_image", self.image_tensor, 3, ["image"])
                    tf.summary.image("image_gen", self.img_gen,3, ["image"])
                    tf.summary.image("input_image", self.image_tensor, 3, ["image_2"])
                    tf.summary.image("rec_x_ema",self.rec_x_ema, 3, ["image_2"])
...
#            with tf.variable_scope(net_name):
#                net = tf.layers.Dense(
#                    units=1 * 1 * 256, kernel_initializer=self.init_kernel, name="fc"
#                )(net)
#                net = tf.layers.batch_normalization(
#                    inputs=net,
#                    momentum=self.config.trainer.batch_momentum,
...
#                )
#                net = tf.reshape(net, shape=[-1, 1, 1, 256])
            net_name = "layer_2"
            with tf.variable_scope(net_name):
            ...
            with tf.variable_scope(net_name):
                ...
                net = tf.nn.leaky_relu(
                    features=net, alpha=self.config.trainer.leakyReLU_alpha, name="tconv5/relu"
                )
    ...
    def discriminator_xz(self, img_tensor, noise_tensor, getter=None, do_spectral_norm=True):
        ...
        with tf.variable_scope("Discriminator_xz", reuse=tf.AUTO_REUSE, custom_getter=getter):
            ...
            with tf.variable_scope(net_name):
                ...
                x = tf.nn.leaky_relu(
                    features=x, alpha=self.config.trainer.leakyReLU_alpha, name="conv4/leaky_relu"
                )
...
#                )
                x = tf.nn.leaky_relu(
                    features=x, alpha=self.config.trainer.leakyReLU_alpha, name="conv5/leaky_relu"
                )

            x = tf.reshape(x, [-1, 1, 1, self.config.trainer.noise_dim ])
            # Reshape the noise
            ...
            with tf.variable_scope(net_name):
                y = layers.conv2d(
                    y,
                    filters=self.config.trainer.noise_dim*2,
                    kernel_size=1,
                    strides=(1, 1),
                    padding="same",
                    kernel_initializer=self.init_kernel,
    ...
    def discriminator_xx(self, img_tensor, recreated_img, getter=None, do_spectral_norm=True):
        ...
        with tf.variable_scope("Discriminator_xx", reuse=tf.AUTO_REUSE, custom_getter=getter):
            ...
            with tf.variable_scope(net_name):
                ...
                net = tf.nn.leaky_relu(
                    features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv1/leaky_relu"
                )
...
#                )
            ...
            with tf.variable_scope(net_name):
                ...
                net = tf.nn.leaky_relu(
                    features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv3/leaky_relu"
                )
#                net = tf.layers.dropout(
#                    net,
#                    rate=self.config.trainer.dropout_rate,
...
#                )
            ...
            with tf.variable_scope(net_name):
                ...
                net = tf.nn.leaky_relu(
                    features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv4/leaky_relu"
                )
#                net = tf.layers.dropout(
#                    net,
#                    rate=self.config.trainer.dropout_rate,
...
#                net = tf.nn.leaky_relu(
#                    features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv5/leaky_relu"
#                )
#                net = tf.layers.dropout(
#                    net,
#                    rate=self.config.trainer.dropout_rate,
...
#                )
    ...
    def discriminator_zz(self, noise_tensor, recreated_noise, getter=None, do_spectral_norm=True):
        ...
        with tf.variable_scope("Discriminator_zz", reuse=tf.AUTO_REUSE, custom_getter=getter):
            ...
            net_name = "y_layer_1"
            with tf.variable_scope(net_name):
                y = layers.dense(y, units=self.config.trainer.noise_dim*2, kernel_initializer=self.init_kernel, name="fc")
            ...
            net_name = "y_layer_2"
            with tf.variable_scope(net_name):
                y = layers.dense(y, units=self.config.trainer.noise_dim, kernel_initializer=self.init_kernel, name="fc")
                y = tf.nn.leaky_relu(features=y, alpha=self.config.trainer.leakyReLU_alpha)
                y = tf.layers.dropout(
                    y,
                    rate=self.config.trainer.dropout_rate,
