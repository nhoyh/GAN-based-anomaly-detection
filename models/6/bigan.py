...
class BIGAN(BaseModel):
        ...
        if self.config.log.enable_summary:
            with tf.name_scope("Summary"):
                ...
                with tf.name_scope("Image_Summary"):
                    ...
                    tf.summary.image("reconstructed", self.reconstruct_ema, 3, ["image_2"])
    ...
    def discriminator(self, noise_input, image_input, getter=None):
        ...
        with tf.variable_scope("Discriminator", custom_getter=getter, reuse=tf.AUTO_REUSE):
            ...
            x_d = tf.reshape(x_d, [-1, 2 * 2 * 128])

            # D(z)
            net_name = "Z_Layer_1"
            with tf.variable_scope(net_name):
                z = tf.layers.Dense(units=128, kernel_initializer=self.init_kernel, name="fc")(
                    noise_input
                )
                z = tf.nn.leaky_relu(
